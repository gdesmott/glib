...
pub fn impl_gflags(input: &syn::DeriveInput) -> TokenStream {
    ...
    let gtype_name = match parse_type_name(&input, "gflags") {
        Ok(v) => v,
        Err(e) => abort_call_site!(
            "{}: derive(GBoxed) requires #[gflags(type_name = \"FlagsTypeName\")]",
            e
        ),
    };

    let get_type = format_ident!("{}_get_type", name.to_string().to_snake_case());
    let (genum_values, nb_genum_values) = gen_genum_values(name, struct_fields);
    ...
    quote! {
        ...
        fn #get_type() -> glib::Type {
            ...
            ONCE.call_once(|| {
                static mut VALUES: [gobject_sys::GFlagsValue; #nb_genum_values] = [
                    #genum_values
